...
private let kVTCompressionPropertyKeyEnableMCTFPrivate = "EnableMCTF"
...
public final class MDKVideoToolboxEncodingProcessor: MDKCaptureFrameProcessing, @unchecked Sendable {
    ...
    private func ensureCompressionSession(
        ...
        if let enableMCTF = resolvedEnableMCTF {
            setSessionProperty(
                session,
                key: kVTCompressionPropertyKeyEnableMCTFPrivate as CFString,
                value: enableMCTF ? kCFBooleanTrue : kCFBooleanFalse,
                label: "EnableMCTF"
            )
        }
        if let hdrConfiguration {
        ...
        sessionConfigurationNotes.append("videoToolboxConfiguredEnableMCTF=\(resolvedEnableMCTF.map(String.init) ?? "default")")
