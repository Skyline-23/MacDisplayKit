...
public final class MDKVideoToolboxEncodingProcessor: MDKCaptureFrameProcessing, @unchecked Sendable {
    ...
    private func encode(
        ...
        let codedDimensions = heightAlignedCodedDimensions(
    ...
    private func ensureCompressionSession(
        ...
        if width != visibleWidth || height != visibleHeight {
            ...
            sessionConfigurationNotes.append("videoToolboxCleanAperture=height-aligned")
        } else {
    ...
    private func heightAlignedCodedDimensions(
        visibleDimensions: SIMD2<Int>,
        sourcePixelFormat: UInt32,
        targetPixelFormat: UInt32
    ) -> SIMD2<Int> {
        guard shouldUseHeightAlignedCodedAperture(
            sourcePixelFormat: sourcePixelFormat,
            targetPixelFormat: targetPixelFormat
        ) else {
    ...
    private func shouldUseHeightAlignedCodedAperture(
        sourcePixelFormat: UInt32,
        targetPixelFormat: UInt32
    ) -> Bool {
        codec == .hevc &&
