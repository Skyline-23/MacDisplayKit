...
public final class MDKVideoToolboxEncodingProcessor: MDKCaptureFrameProcessing, @unchecked Sendable {
    ...
    private var scaler: MDKMetalBilinearScaler?
    ...
    public init(
        ...
        self.scaler = (codec == .proResProxy) ? nil : device.map { MDKMetalBilinearScaler(device: $0) }
    ...
    private func stageAndEncode(
        ...
        if frame.pixelFormat != targetPixelFormat {
            ...
            guard let scaler = ensureScaler() else {
                stagingSubmissionGroup.leave()
                releaseStagingSlot(identifier: slotIdentifier)
    ...
    private func ensureScaler() -> MDKMetalBilinearScaler? {
        if let scaler {
            return scaler
        }
        guard let device else {
        ...
        let scaler = MDKMetalBilinearScaler(device: device)
        self.scaler = scaler
        return scaler
    }
