...
final class MDKMetalBGRAToYCbCrConverter {
    ...
    private let combinedSampled420Pipeline: any MTLComputePipelineState
    ...
    init(device: any MTLDevice) throws {
        ...
        guard let lumaFunction = library.makeFunction(name: "bgraToYCbCrLuma"),
              ...
              let combinedSampled420Function = library.makeFunction(name: "bgraToYCbCrCombinedSampled420"),
            ...
            throw MDKMetalColorConversionError.functionMissing(
                "bgraToYCbCrLuma/bgraToYCbCrChroma/bgraToYCbCrCombinedSampled420/overlayCursorOnBGRA"
            )
        }
        ...
        do {
            ...
            combinedSampled420Pipeline = try device.makeComputePipelineState(function: combinedSampled420Function)
    ...
    func encode(
        ...
        let canUseCombinedSampled420Path =
        ...
        if canUseCombinedSampled420Path {
            ...
            computeEncoder.setComputePipelineState(combinedSampled420Pipeline)
            dispatch(
                encoder: computeEncoder,
                pipeline: combinedSampled420Pipeline,
    ...
    kernel void bgraToYCbCrCombinedSampled420(
        texture2d<float, access::sample> sourceTexture [[texture(0)]],
        ...
        for (uint offsetY = 0; offsetY < 2; ++offsetY) {
            for (uint offsetX = 0; offsetX < 2; ++offsetX) {
                ...
                float2 sampleCoordinate = (float2(lumaPixel) + float2(0.5)) / lumaSize;
