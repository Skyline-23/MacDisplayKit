...
private final class MDKEncodedTileStreamProcessor: MDKEncodedCaptureProcessorRuntime, @unchecked Sendable {
    ...
    private let deferredTileSubmissionQueue = DispatchQueue(
        label: "com.skyline23.MacDisplayKit.encoded-tile.shared-session.deferred-submit"
    )
    ...
    func process(
        ...
        do {
            for laneIndex in regions.indices {
                guard laneIndex == regions.startIndex else {
                    deferredTileSubmissionQueue.async { [self, frame, releaseCoordinator] in
                        do {
    ...
    func finalize() -> MDKCaptureFrameProcessingSummary? {
        deferredTileSubmissionQueue.sync {}
        return summarize([processor.finalize()].compactMap { $0 })
    }
    ...
    private func summarize(_ summaries: [MDKCaptureFrameProcessingSummary]) -> MDKCaptureFrameProcessingSummary? {
        ...
        var notes = [
            ...
            "videoToolboxEncodedTileStreamAdmissionMode=deferred-secondary-tile"
        ]
...
public actor MDKEncodedCaptureSession {
    ...
    private static func makeSourcePreparation(
        ...
        case .skyLightDisplayStream:
            ...
            if ProcessInfo.processInfo.environment["MDK_SKYLIGHT_DISPLAY_STREAM_PREFLIGHT_FIRST_CLEAN_FRAME_WARMUP"] == "1" {
                do {
                    ...
                    diagnosticNotes += [
                        "skyLightPreflightWarmupMode=first-clean-complete-frame",
