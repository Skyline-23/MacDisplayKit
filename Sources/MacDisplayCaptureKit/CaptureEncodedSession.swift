...
private actor MDKEncodedCaptureOrderedDrainCoordinator {
    ...
    private let coalescedFrameHandler: @Sendable (UInt64) -> Void
    ...
    init(
        ...
        coalescedFrameHandler: @escaping @Sendable (UInt64) -> Void,
        initialCredits: Int
    ) {
    ...
    private func start(callbacks: MDKEncodedCaptureCallbacks?) async throws {
        ...
        let orderedDrainCoordinator = usesOutputDrivenDrainCredits
            ? MDKEncodedCaptureOrderedDrainCoordinator(
                ...
                latestFrameMailbox: latestFrameMailbox,
                failureHandler: failureHandler,
                coalescedFrameHandler: { [weak self] sourceDisplayTime in
                    guard let self else {
                        return
                    }
                    Task {
            ...
            guard pendingFrameTracker.tryAcquire(limit: maximumPendingFrameCount) else {
                Task {
                    ...
                    if let replacedDisplayTime {
                        if orderedDrainCoordinator != nil {
                            await self.handleSourceFrameCoalesced(
