...
private final class MDKSpatialTileOutputGate: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.skyline23.MacDisplayKit.encoded-capture.spatial-tile-output")
    private let expectedTileCount: Int
    ...
    init(
        ...
        self.expectedTileCount = max(expectedTileCount, 1)
        self.outputHandler = outputHandler
    }

    func emit(_ frame: MDKEncodedFrame) {
        let completedFrames = queue.sync {
            let metadata = frame.tileMetadata
            guard metadata.tileCount > 1 else {
                return [frame]
            }

            let groupID = metadata.frameGroupID == 0 ? frame.sourceSequenceNumber : metadata.frameGroupID
            if pendingGroups[groupID] == nil {
                pendingGroups[groupID] = [:]
                groupOrder.append(groupID)
            }
            pendingGroups[groupID]?[metadata.tileIndex] = frame

            while groupOrder.count > expectedTileCount * 3 {
                let evictedGroupID = groupOrder.removeFirst()
                pendingGroups.removeValue(forKey: evictedGroupID)
            }

            guard let group = pendingGroups[groupID],
                  group.count >= max(Int(metadata.tileCount), expectedTileCount) else {
                return []
            }

            pendingGroups.removeValue(forKey: groupID)
            groupOrder.removeAll { $0 == groupID }
            return group.keys.sorted().compactMap { group[$0] }
        }

        for completedFrame in completedFrames {
            outputHandler(completedFrame)
        }
    }

    func pendingGroupCount() -> Int {
        queue.sync { pendingGroups.count }
    }
}
...
private final class MDKSpatialTileEncodedCaptureProcessor: MDKEncodedCaptureProcessorRuntime, @unchecked Sendable {
    ...
    private let outputGate: MDKSpatialTileOutputGate
    ...
    init(
        ...
        let outputGate = MDKSpatialTileOutputGate(
        ...
        self.lanes = regions.enumerated().map { tileIndex, region in
            MDKVideoToolboxEncodingProcessor(
                ...
                outputHandler: { frame in
                    outputGate.emit(frame)
                },
                failureHandler: failureHandler,
                ...
                sourceRegion: region,
                releaseSourceFrameAfterOutput: true
            )
        }
    }
    ...
    private func summarize(_ summaries: [MDKCaptureFrameProcessingSummary]) -> MDKCaptureFrameProcessingSummary? {
        ...
        var notes = [
            "videoToolboxSpatialTileCount=\(lanes.count)",
            "videoToolboxSpatialTilePendingGroups=\(outputGate.pendingGroupCount())"
        ]
