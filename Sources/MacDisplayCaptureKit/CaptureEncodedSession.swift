...
private actor MDKEncodedCaptureOutputDrivenDrainCoordinator {
    ...
    private let steadyStateCredits: Int
    private var startupOutputCallbacksRemaining: Int
    private var activeCreditLimit: Int
    ...
    init(
        ...
        startupCredits: Int,
        steadyStateCredits: Int,
        startupOutputCallbacks: Int
    ) {
        self.processor = processor
        self.pendingFrameTracker = pendingFrameTracker
        self.latestFrameMailbox = latestFrameMailbox
        self.failureHandler = failureHandler
        self.steadyStateCredits = max(steadyStateCredits, 1)
        self.startupOutputCallbacksRemaining = max(startupOutputCallbacks, 0)
        self.activeCreditLimit = max(startupCredits, 1)
        self.availableCredits = max(startupCredits, 1)
    }
    ...
    func replenishCredit() {
        if startupOutputCallbacksRemaining > 0 {
            startupOutputCallbacksRemaining -= 1
            if startupOutputCallbacksRemaining == 0 {
                activeCreditLimit = steadyStateCredits
                availableCredits = min(availableCredits, activeCreditLimit)
            }
        }

        availableCredits = min(availableCredits + 1, activeCreditLimit)
        scheduleIfNeeded()
    }

    private func scheduleIfNeeded() {
        while availableCredits > 0, !queuedFrames.isEmpty {
...
public actor MDKEncodedCaptureSession {
    ...
    private func start(callbacks: MDKEncodedCaptureCallbacks?) async throws {
        ...
        let usesOutputDrivenDrainCredits =
            ...
            configuration.targetFrameRate >= 100 &&
            configuration.resolvedEncodedHDRConfiguration?.transferFunction == .smpteSt2084PQ
        let outputDrivenDrainCoordinator = usesOutputDrivenDrainCredits
            ? MDKEncodedCaptureOutputDrivenDrainCoordinator(
                processor: processor,
                pendingFrameTracker: pendingFrameTracker,
                latestFrameMailbox: latestFrameMailbox,
                failureHandler: failureHandler,
                startupCredits: maximumPendingFrameCount,
                steadyStateCredits: min(maximumPendingFrameCount, 2),
                startupOutputCallbacks: 2
            )
            : nil
        if let outputDrivenDrainCoordinator {
            processor.installSourceDrainCreditHandler {
                Task {
                    await outputDrivenDrainCoordinator.replenishCredit()
                }
            }
        }
        let source = sourceFactory(configuration, sourcePreparation) { [weak self, weak processor] frame in
            ...
            guard pendingFrameTracker.tryAcquire(limit: maximumPendingFrameCount) else {
                Task {
                    ...
                    if let replacedDisplayTime {
                        if outputDrivenDrainCoordinator != nil {
            ...
            if let outputDrivenDrainCoordinator {
                Task {
                    await outputDrivenDrainCoordinator.enqueue(frame)
                }
            } else {
                MDKProcessMailboxAwareSourceFrame(
                    frame,
    ...
    private func handleSourceFrameCoalesced(sourceDisplayTime: UInt64, runtimeGeneration: UInt64) {
        ...
        emitEvent(
            .init(
                kind: .coalescedFrame,
                message: "Source frame coalesced before processing to preserve latest-frame freshness.",
                sourceDisplayTime: sourceDisplayTime
            )
        )
    }
